  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        let frame = 0;
        camInst = new Camera(vid, {
          onFrame: async () => {
            if (destroyed) return;
            frame++;
            if (frame % 2 === 0) await hands.send({ image: vid });
            else await face.send({ image: vid });
          },
          width: 640, height: 480,
        });
        await camInst.start();

        if (camRef.current && vid.srcObject) {
          camRef.current.srcObject = vid.srcObject;
    ...
    init();
    return () => { destroyed = true; camInst?.stop(); };
  }, []);

  useEffect(() => {
    if (screen === 'title' || screen === 'playing') {
      const v = videoRef.current, c = camRef.current;
      if (v && c && v.srcObject) { c.srcObject = v.srcObject; c.play().catch(() => {}); }
    }
  }, [screen]);
